...
void DoHydraulicModel(bool silent,
	...
	// [sean]... can't find where any of these variables are defined... can sort of guess...
	...
	*/
	...
	void HydraulicModel::readin(bool silent, bool callRewet,
		...
		{  // [sean] beginning of readin return
			...
			double xylemScalar = treesParams.xylemScalar;  // [sean] don't know what this is
	...
	for (i = ShootModules+2; i < ShootModules+RootModules+2; i++)
	{
		if (psimin[i][1] != -8.0)
		{
			// [sean] I don't understand this.. why is psi_soil a function of root area?
	...
	if (delta > 0.005 || treesParams.updatedHydraulics == true)
	{
		...
		// [sean] i don't know how this bool is being used...
	...
	{
		if (treesParams.forceRefilling == true)  // [sean]  why do we need a "forceRefilling" param?
		{
			...
			for (i = 1; i <= totmodules+1; i++)
			{
				if ( i != (ShootModules+1))  // skip over "hidden" module  // [sean] hidden module? Litter? Surface?
				{
					if (i <= ShootModules)
					{
						...
						// [sean] this stores the same value in the same matrix, but in the first column (why?)
						psimin[i][0] = soilpsiavg[t];
					}
					else if (i > (ShootModules+1)) // [sean] meaing if loop has passed onto the soil layers
					{
		...
		// [sean] don't know what this bool is for.  It is hardcoded in 3 places in simulator.cpp
		...
		if (!silent)  // [sean] ??
		{
			if (xylemScalar < 1.0)  // [sean] xylemScalar adjustes "starting" Ks_max
			{
				...
				xylemScalar = 1.0;  // [sean] huh?  setting to 100 when it is close to 100... why?
			}
		...
		pdecrement = 0.01;  // [sean] don't know what these are. Fixed indent
		cincrement = 0.01;  // [sean] should probably move these down to the first Do-While loop
		...
		for (i = 1; i <= totmodules+1; i++) // [sean] looping through modulules again
		{
			...
			if (i != (ShootModules+1))  // skip over "hidden" module
			{
				...
				do //[sean] ...probably a residual from visual basic.  I cleaned it up some
				//[scott or john] loop calculates new plc's based on new predawn and finds new b
				{
					...
					else  // [sean] stops when psi_xylem = psi_pd... and then plc is calculated for that pressure (below)... but why the loop (above)?
					{
						//lateral shoot(s) infected with bluestain fungi
						...
						{
							// [sean] updates plc at pressure[k] for roots (note: exactly the same plc eqn... so don't need if-then-else)
							plc[k] = (1.0-xylemScalar*exp(-pow((-pressure[k] / bsat[i][1]), ccsat[i][1]))) * 100.0;
						}
					}  // [sean] end of previous (redundant) if-then-else (fixed indenting and spacing above)
					...
					if ( plc[k] < 66.0 )
					{
						// [sean] fabs is a math(lib) fxn (Nathan's help)... returns absolute value of a double ("-psi when plc=60")
				...
				{ //loop finds new c
					...
					for (k = 1; k <= ktotal; k++)
					{
						...
						{
							//lateral shoot(s) infected with bluestain fungi
							//[sean] if and else functions are exactly the same and therefore redundant
							plcweib[k] = (1.0 - exp(-pow((-pressure[k] / newb[i][1]), newc[i][1]) )) * 100.0;
						}
						else
						{ // [sean] same fxn as under if statment (redundant)
							plcweib[k] = (1.0 - exp(-pow((-pressure[k] / newb[i][1]), newc[i][1]) )) * 100.0;
						}
		...
		for ( i = 1; i <= totmodules+1; i++)
		{
			if ( i != ShootModules + 1 ) //[sean] avoiding surface layer?
			{
				...
				if (i <= ShootModules)
				// [sean] again... equations in if and else appear identical... ??
		...
		preSetup( md_raw, treesParams.Md,  e_raw, ev,  la_raw,
			...
			readin(silent, callRewet, soilpsi,  soilpsiavg,  ks, pressure,  plc,  l_raw, ksat,
				...
				initialize(r, pe, b, b1, n, n1, ks, ws_, rssoil, cf, vu, vl, saturatedKs, cpp, wnu, wu, wnl,
					...
					//***************************************************************************************************
					// [sean] can't find what "PM" is... not a function in the code anywhere
						...
						do
						{
							...
							do
							{
								...
								for (  i = 1; i <= ShootModules; i++ )
								{
									...
									jl[i][2] = evap[ii] * al[i]; //sets evaporation rate [sean] [ii] is an declared int
								}
								...
								for (i = 1; i <= ShootModules; i++ )
								{
									...
									//[sean] tho seems like a strange and probably incorrect assumption that there will be no capactiance exchange
									if ( p[i][0] > ip[i][1] )
									{
								...
								//[sean] now, moving to the next lower element (upper axial element and its associated nodes)
								{
									...
									if ( p[i + 1][0] > ip[i][0] )
									{
										...
										//[sean] below comment should have been moved up!.. but yeah... should be zero
										//[Strictly][this should be zero][but set to arbitrary small #?
								...
								//[sean] and columns (second array) holding numbers for axial (0) or lateral (1) elements
								{
									...
									//[sean] I don't see why we'd want to either... use Darcy's law
								...
								//get upper and lower k's for lateral shoot elements
								{
									...
									//phi for upper half [sean] don't know what this is needed for...
								...
								//[sean] I don't see how this "ties up loose ends"
								...
								for (i = 1; i <= tnode; i++ )
								{
									...
									// [sean] why are not zeroing index position [0][0]????
								...
								for ( k = 1; k <= ShootModules; k++ )
								{
									...
									//dF[k,0]/dY[k-1,0]
									// [sean] [j - 2] = -1 (!!!!outside the range of the array!!!!!!!!!!!!!)
