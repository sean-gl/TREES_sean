...
//this is where roots are disconnected
	...
	do //coupled plant water balance and carbon assimilation
	{
		...
		if(ModelStatus == 0)
		{
			...
			//[sean] 'calc_gvc' is an overloaded function... check 'process_functions' to see what it does based on the inputs given here..
			//[sean] this is the first defined calc_gvc fxn in 'process_fuctions'... considers only max_gs, VPD, and LAI
			//[sean] (not PAR, not PSI)
		...
		else
		{
			...
			ti.get_time(year, yday, hour, min);
			treesParams.useRefilling = false;  // [sean] default = false
			callRewet = false;                 // [sean] default = false

			if (ts == 1)   // [sean] commenting out and changing to below to use the setting in *.p file
			// if (ts == 1 && treesParams.useRefilling == true)
			{
				callRewet = true; //[sean] changing to false (default = true)
				treesParams.useRefilling = true; // [sean] changing to false (default = true)
				callHydraulicModel = 0;
			}
			else if (fabs(treesParams.xylemScalar-state.get_val_at(XYLEM)) > 0.0 &&
			fabs(treesParams.xylemScalar-state.get_val_at(XYLEM)) < 0.01 )
			{
				if ((hour+min) == 5.0)  // [sean] commenting out and changing to below to use the setting in *.p file
				//if ((hour+min) == 5.0 && treesParams.useRefilling == true)
				{
					callRewet = true;  //[sean] changing to false (default = true)
			...
			{
				callRewet = true; // [sean]  fixing the indenting here
				callHydraulicModel = 0;
				treesParams.useRefilling = true;
			} // [sean] fixed indenting
			...
			else
			{
				callRewet = false;
				callHydraulicModel = 0;
				treesParams.useRefilling = false; // [sean] commented out... not sure why this is necessary
			}
			//Reset to saved K's P's and Weibulls until the algorithm converges
			for (mm = 1; mm <= totmodules+1; mm++)
			{
				if ( mm != ShootModules+1)  // skip over "hidden" module   [sean] huh?
				{
					ks2[mm][1] = ks_in[mm][1];
					ks2[mm][0] = ks_in[mm][0];
			...
			if (HydraulicModelFailCond == 0) //[sean] 0=sucessful run.  1=fail
			{
				if (isnan(Kl))
				{
		...
		if(ModelStatus == 1)
		{
			...
			//[sean] this is the version of the overloaded fxn that takes leafpsi and D
			//[sean] the calc_gvc fxn (10-argument version), as it's run here, will deterimine gs response
			//[sean] to these driver inputs.  This function is located in process_functions.cpp
...
//
	...
	capRise = capRiseScalar*capillary_rise(Zw, bubbling_pressure, pore_size_index, ks)*
							...
							// [sean] send to screen
							// [sean] capillary_rise fxn will only move water upward from a water table... not from one layer to another
							// cout << capRise << "   " << (Et-soil_evaporation1-soil_evaporation2-canopy_evaporation) << endl;
	...
	if (capRise < 0.0)
	{
		capRise = 0.0;
							...
							// [sean]
							// cout << (Zw - capFringe) << "   " << cumDroot[0] << endl;

	if ((Zw - capFringe) >= cumDroot[0])
	{
		capRise_shallow = capRise * (rootDepth[0])/(Zw-capFringe);
	//	cout << "capRise_shallow = " << capRise_shallow << endl; // [sean]
	}
	else
	{
		capRise_shallow = 0.0;
	...
	for (i = 1; i < rmodules-1; i++)
	{
		...
		//cout << "capRise_mid = " << capRise_mid << endl; //[sean]
	...
	//cout << "capRise_deep = " << capRise_deep << endl; //[sean]
...
//If NSC recovers to at least 0.5% of live tissue then restore refilling
			...
			{
				treesParams.useRefilling = true; // [sean] commenting out
				if (!silent)
				{
			...
			{
				treesParams.useRefilling = true; // [sean] commenting out
        if (!silent)  // [sean] correcting indent
				{
					cout << "*** Re-filling turned on ***" << endl;
				}
			} // [sean] correcting indent
		}
	}
