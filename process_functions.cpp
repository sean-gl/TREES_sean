...
#include <stdio.h>
#include <math.h>
#include <unistd.h> //[sean] sleep fxn
#include "constants.h"
...
void compute_GSI_LAI(int ts,
	...
	vector<double> &GSI_vector)
	{
		...
		if (hour+(double)min/100.0 == 23.5 && ts >= 48)
		{
			...
			bgc.updateLeafCarbonNitrogenPools(treesParams, delta_lai, RL, N_neg_fract, N_neg_demand, N_pos_demand);
...
double vpd)
*/
double calc_GSI(double photoPeriod,	double temperature, double gsi_lwp)
{
...
//ref temp in K - observed
double sky_emissivity(double tK, double fcloud)    //fraction cloud cover
{
	double emm;
...
double diffuse_flux(const Time& t, double Qt,	double zenith)
{
	int year, jday, hour, min;
...
//current time
//Qt, measured radiation flux W m-2 (ground)
//zenith angle (radians)
//ref args
void rad_partition(const Time& t, double Qt, double zenith,	double& Qob,
	double& Qod)
	{
...
// [sean]  be careful!! calc_gvc is an overloaded function... watch your inputs to know what fxn is running
...
double calc_gvc(double Kl,
	...
	double pair)
	{
		...
		if (soilpsi < 0.0 && Kl > 0.0 && leafpsi < soilpsi && !isnan(leafpsi))
		{
			...
			//[sean] default gs fxn
			//[sean] why is the accerlation of gravity in meters but psi in MPa?
			//[sean] soilpsi - leaf psi represents the gradient... as the gradient steepens
			//[sean] (leafpsi becomes more negative) gs increases because, all else being equal, the flow
			//[sean] of water increases..., which is the opposite response we
			//[sean] should expect to protect the xylem from damage... what to do?
			// gs = (Kl/D_molar) * (soilpsi - leafpsi - (canopy_ht*(9.81)*0.001)); //[sean] default

			//[sean] keep all except soilpsi constant, change leafpsi
			//[sean] default behavior = gs
			// D_molar = 0.03;
			// soilpsi = -0.15;
			// canopy_ht = 1.34;
			// Kl = 2.0;

			//[sean] first... use default fxn to calculate gs based on soil-to-leaf pressure gradient
			gs = (Kl/D_molar) * (soilpsi - leafpsi - (canopy_ht*(9.81)*0.001)); //[sean] default
...
double capillary_rise(double water_table_depth,
	...
	double sat_hydraulic_cond)
	{
		...
		{
			return(0.0); // [sean] this is why we can't use this fxn to move water between layers (water only moves up from the water table)
		}
...
void infiltration(double input,
	...
	double ks)
	{
		...
		//[sean] keep
		//Set 30 second time steps if soils are wetter than field capacity
		...
		{
			//if soils are dry then no need to call drainage
			kiter = 0;
			kinter = 1.0;
		...
		//[sean... and following comments] here's is how this while loop works....
		// first, the change in water content of the layer above the [i] layer is considred (delta_Sr from previous iteration of this loop)
		// if this content is more than the total amount of water than the layer can hold, then this water
		// is passed directly to the double 'bypass'.  'bypass' is the variable that holds all **saturated** flow into
		// the **next lower** layer, i.e. it is not added to the [i] layer
		// if the water passed from the above layer (delta_Sr) is less than the amount that can be held, a small portion is still
		// passed to "bypass", i.e. a fraction is still considred saturated flow (see pow fxn below with 'bpp')
		// this saturated flow amount (stored in bypass) is then subtracted from the water transfered from the
		// above layer (delta_Sr).  It's not clear to me why this needs to be done, but it may be that the pressure
		// functions used in the unsaturated flow fxns (below) should not be given water contents above saturation.
		// this new updated delta_Sr (with all saturated water removed and put into 'bypass') is then added to the
		// [i] layer, but not directly... it is added to a new variable "updatedTheta".  This newly caclulated water
		// content for layer [i] (updatedTheta) is then passed to the unsaturated drainage functions below.
		// these functions then calculate (via discrete Richards eqn) the unsaturated flow from the [i] layer,
		// which is stored in the variable 'drainage'. This unsaturated flow out of the [i] layer is then
		// finally subtracted from thetaSoil[i].
		// now... all the saturated flow (from the above layer) and all the saturated flow out of the [i] that
		// had been stored in 'bypass' is added to delta_Sr (the total flow into the **next** layer)
		// Then the loop reiterates.
		// As such, this new 'delta_Sr' (with bypass added to it) is considered.
		// free water (satruated flow) is adde to bypass. Unsaturated flow out of it is calculated and saved as
		// drainage (which is subtracted from it) and delta_Sr is relcaluated again for the next layer... and so on

		//[sean] replacing the if-else statement above with the following two lines...
		//[sean] so drainage is always called, and the unsat fxn runs in all cases
		// kiter = 30; //intervals of 30 seconds
		// kinter = 30.0;
		...
		while (kiter > 0)
		{
			...
			//[sean] this loop cycles **down** the profile and transfers water from upper layers to lower layers (...mostly)
			for (int i = 0; i < rmodules; i++)
			{
				bypass = 0.0;  //[sean] note that this sets bypass to zero, i.e., bypass only passes water down the profile
				...
				//[sean] I think thetaSoil is divided by rootdepth and mineral fraction
				//[sean] to obtain the fraction of water that is not roots or mineral
				updatedTheta = thetaSoil[i]+delta_Sr/(rootDepth[i]*mineral_fraction);
				//[sean] we've just calcualted how much saturated-flow water has passed from the soil layer above ("bypass")
				// [sean] and we've reclculated the water content of the layer (saturated flow into it - bypass)
				// [sean] if the recalculated amount of water in the layer happens to be more than the layer
				// [sean] can hold (> "porosity") then all this extra water is simply added to "bypass"
				// [sean] and the water conent of the soil layer set to maxmial ("porosity")
				drain = 0.0;
				...
				//[sean] thus, we've put all the saturated flow from from previous layer and the [i] layer and put it in 'bypass',
				//[sean] which will eventually be added to the flow into the next lower layer (at end of loop)
				//[sean] the rest of the code below deals with unsaturated flow via a discrete Richards eqn (iteration of Darcy's law in short time steps)
				//[sean] note that the if statment below is "< rmodules-1"... and as such, this 'if' statement will not
				//[sean] run for the bottom-most layer (remember that the top layer is 0, not 1), i.e., no water is
				//[sean] transfered from the bottom layer to another layer (it just disappears)
				if (i < rmodules-1)
				{
				...
				//[sean] this runs only for the bottom-most layer
				{
				...
				//[sean] adding scaling factor to drain (unsat flow) to move more water between layers at each iteration
				delta_Sr -= (drain*1);
				// delta_Sr -= drain; [sean] silencing this and replacing with line above

				//[sean] finally, the [i] soil layer is updated
				thetaSoil[i] += delta_Sr/(rootDepth[i]*mineral_fraction);

				//[sean] adding this to prevent theta from becoming negative
				if(thetaSoil[i] < 0) { thetaSoil[i] = 0; }

				//[sean] again... adding free water to the double 'bypass'
				...
				//[sean]
				// cout << "root_module = " << i << "  drain(unsat) = " << drain << " bypass = " << bypass << " updated_theta = " << updatedTheta << "  thetaSoil[i] = " << thetaSoil[i] << endl;

				//[sean] bypassFlow is only used for unsaturated flow
				//[sean] this is calculated for the flow from one soil layer to another
				//[sean] it is remembered for thenext iteration by additin it to delta_Sr here at the end of the loop
				delta_Sr = drain + bypass;
				bypassFlow[i] += bypass;

			} //[sean] end of for loop
			drain = delta_Sr = 0.0;



			//[sean] my for loop for lifting water up throught the profile via unsatruated flow...
			//[sean] this is the call to my unsaturated drainage fxn that runs for all layers except for the very bottom layer
			//[sean] it only moves water upward from layer to layer (not downward)
			for (int i = rmodules-1; i > 0; i--)
			{
				drain = unsat_upward_flow_sean(rootDepth[i-1], rootDepth[i], pore_size_index,
					ks/kinter, thetaSoil[i-1], thetaSoil[i],
					field_capacity, residual, capFringe,
					porosity, mineral_fraction);

				//[sean] adding scaling factor to drain (unsat flow) to move more water faster between layers at each iteration
				delta_Sr = (drain*1)*-1;
				//[sean] adding the value of drain to the upper layer that was run in the usat_upward_flow fxn run

				// cout << "root_module = " << i-1 << "  sean_drain = " << drain << endl;
				// cout << "root_module = " << i-1 << "  delta_Sr = " << delta_Sr << endl;
	...
	//			 delta_Sr = 0; //[sean] turn off to stop the the upward movement of water
				...
				thetaSoil[i-1] += delta_Sr/(rootDepth[i-1]*mineral_fraction);
				thetaSoil[i] -= delta_Sr/(rootDepth[i]*mineral_fraction);


				//[sean] adding this to prevent theta from becoming negative
				if(thetaSoil[i-1] < 0) { thetaSoil[i-1] = 0; }

//			cout << "root_module = " << i << " sean_drain = " << drain << " post_thetaSoil[i-1] = " << thetaSoil[i-1] << endl;
//			cout << "root_module = " << i << " sean_drain = " << drain << " thetaSoil[i-1] = " << thetaSoil[i-1] << "  thetaSoil[i] = " << thetaSoil[i] << endl;
			} // end of sean for loop
			//[sean] return these to zero
			drain = delta_Sr = 0.0;
	...
	} //[sean] end of infiltration fxn




// [sean] this function (and most others) is declared in simulator2.h (there is no process_fuctions.h).
...
double unsat_zone_drainage(double dZ1,
	...
	double mineral_fraction)
	{
		...
		double Z1, Z2;  // [sean] these are contributing and recieving layers, respectively... but are not used
		int iter;

		//[sean] commenting this bit out (as it's writen it returns zero if the lower layer has no roots)
		// we want to transfer water whether or not there are roots present.  I think the problem with this
		// is that when we mulitply soil water content by dZ* (the axial root length and also the soil layer thickness), if
		// dZ* is zero, then so too will theta and suz.  ...need to deal with this with a if statement below...
		// if (dZ2 <= 0.0)
		// {
		// 	return(0.0);
		// }

		//[sean] commenting this out (as it's writen it returns zero if the upper layer has few roots)
		// if (dZ1 < 0.0001)
		// {
		// 	return(0.0001);
		// }

		//[sean]  commenting this out.  As it's written it will not allow flow if upper layer is drier than field capacity
		...
		else if (theta1 > 0.67*porosity) //[sean] ...etc., shorter time steps the farther from field capacity (change theta1 for theta2)
		{
			iter=6;
			ko = ko/6.0;
		}
		...
		//[sean] these values (Z1 and Z2) are commented out below and not used... hummm
		Z1 = 0.5*dZ1;
		Z2 = 0.5*(dZ1 + dZ2);
		...
		for (int i = 0; i < iter; ++i)
		{
			...
			//[sean] psi1 and psi2 are always expressed as positive values and in units of m head
			psi1 = air_entry_pressure*pow(pow(S,-1.0/m)-1,1.0/n);

			//[sean] now do the same, but for the lower soil layer
			...
			//[sean] just need to calculate pressure at other side of the layer (k is calculated above)
			psi2 = air_entry_pressure*pow(pow(S,-1.0/m)-1.0,1.0/n);

			//[sean] have pressure gradient (psi2-psi1) and ku... now calculate linear (Darcy) flow through layer in short time steps
			//[sean] at this point if the bottom layer is wetter than the upper layer this equation will return a negative value for 'drain'
			...
			//[sean] now that Q (drain) has been calculated... add this to to "totalDrain" (set to zero above)...
			//[sean] then subtract drain from soil water content of top layer (suz1) and add to bottom layer (suz2)
			//[sean] then, finally, recalculate theta values for the top and bottom layers
			totalDrain += drain;  //[sean] at every step in this iteration, 'drain' should get smaller and smaller
			suz1 -= drain;        //[sean] subtract transfered water from upper layer temperary variable (suz1)
			suz2 += drain;        //[sean] add transfered water to lower layer temperary variable (suz2)
			theta1 = suz1/(dZ1*mineral_fraction);  //[sean] update upper and lower thetas
			theta2 = suz2/(dZ2*mineral_fraction);
		}
		//[sean] if the final fraction of water being moved from the upper layer to the lower exceeds
		//[sean] the total amount that can be transfered, then transfer the maximum fraction
		...
		//[sean] this is my function to move water upward in the column.  It is essentially the inverse of the
		//[sean] previous function (above).  I have also added it the simulator2 header file
		//[sean] At assumes that 'i' starts at the very last soil layer (rather than the first)
		//[sean] therefore, this needs to run in a 'for' loop that starts with '(int i = rmodules-1; i>0; i--)'
		double unsat_upward_flow_sean(double dZ1,
			double dZ2,
			double pore_size_index,
			double ko,
			double theta1,
			...
			double mineral_fraction)
			{
				//[sean] changing fc to fc1 and fc2 so it can be calculated for both soil layers
				double drain, totalDrain, ku, S, n, m, fc1, fc2, suz1, suz2, psi1, psi2;
				double Z1, Z2;  // [sean] these aren't used
				int iter;
				...
				else if (theta2 > 0.67*porosity)
				{
					iter=6;
					ko = ko/6.0;
				}
				else if (theta2 > field_capacity)
				{
					iter = 3;
					ko = ko/3.0;
				}
				...
				fc1 = field_capacity*dZ1*mineral_fraction;  //[sean] changed to give fc for each layer
				fc2 = field_capacity*dZ2*mineral_fraction;  //[sean] changed to give fc for each layer

				for (int i = 0; i < iter; ++i)
				{
					//[sean] this gives the fraction of total soil volume that is water in the entire layer that is not mineral for the top and bottom layers
					suz1 = theta1*dZ1*mineral_fraction;
					suz2 = theta2*dZ2*mineral_fraction;
					n = pore_size_index + 1.0;
					m = pore_size_index / n;

					//[sean] calculating the percent saturation for the above layer
					S = (theta1 - residual) / (porosity - residual);
					if (S <= 0.001)
					{
						S = 0.001;
					...
					//[sean] keeping Scott's changes here (n=0.5)... calc soil k between layers...
					ku = ko * pow(S, 0.5) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0);

					//calculate soil water potential for the above layer
					//[sean] psi1 and psi2 are always **expressed as** positive values
					psi1 = air_entry_pressure*pow(pow(S,-1.0/m)-1.0,1.0/n);

					//[sean] now do the same, but for the lower soil layer (contributing layer)
					//calculate soil water potential for the lower soil layer
					S = (theta2 - residual) / (porosity - residual);
					if (S <= 0.001)
					{
						S = 0.001;
					...
					//[sean] differs from previous function... calculate ku for bottom layer, rather than top layer
					// ku = ko * pow(S, 0.5) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0);

					//[sean] calculate a mean ku considering both upper and lower layers (assuming ku = soil conductance)
					ku = (ku + (ko * pow(S, 0.5) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0)))/2;

					//[sean] calculate pressure for the lower soil layer
					psi2 = air_entry_pressure*pow(pow(S,-1.0/m)-1.0,1.0/n);

					//[sean] Darcy
					drain = ku*((psi2-psi1)/(0.5*(dZ1+dZ2))+1.0);
					//drain = ku*((psi2-psi1)/(0.5*(dZ1+dZ2))-1.0); //[sean]
				...
				// [sean] for the upward movement of water, allow only negative values of totalDrain
...
// [sean] this fxn is run only for the most bottom soil layer
//[sean] what is Zw?  Has something to do with roots... volume?  Width?
//[sean] this function calculates unsat flow out of the bottom layer
//[sean] the theta for the layer is not recalculated after determining 'drain' (Darcy's Q), but
//[sean] we will need to do this if we want water to move upward out of this layer... I think...
...
double unsat_zone_drainage(double Zw,
	...
	double mineral_fraction)
	{
		...
		if (S > 0.0 && suz > fc) //[sean] leave this as is.  Upward movement was handeled with previous fx.  This is drainiage downward to the watertable
		{
			n = pore_size_index + 1.0;
			m = pore_size_index / n;
			ku = ko * pow(S, n) * pow(1.0 - pow(1.0 - pow(S, 1.0/m), m), 2.0);
