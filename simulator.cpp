...
void simulator(Data_Store& in_data,
		 ...
		 trees_params treesParams,  // [sean] stuct object that contains parameter place holders (declared in sim2.h)
		 Data_Frame& Ecframe,
		 Data_Frame& NEEframe,
		 double& sd_err1,
		 double& sd_err2,
...
// [sean]  this is what will be called in most cases for my simulations...
//---------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------
void simulator(Data_Store& in_data, //[sean] not sure why these are refrences but current_params is a pointer
		 State_Store& state,
		 Parameter* current_params,
		 ...
		 ofstream& hydrout, ofstream& lfareaout)
{
	...
	int i, j, k;
    int l;
	Time ti;  // [sean] user-defined (time_keeper). returns year, day, hour, min
	double u_ref, t_ref, D_ref, precip, Qpar, t_canopy, D_canopy, CO2_atm, Ecobs;
	double p_atm, t_surface, t_soil, t_root, Zw, NEEobs;
	double xylem_capacity_multiplier, Al_init;
	double thetaSoil[ULAT], tempSoil[ULAT]; //[sean] number of soil layers not to exceed
...
//These variables interface with the HydraulicModel model
	...
	for (i = 1; i <= 21; ++i) //[sean] why 21?  Puts a zero into each index of vector GSI_vector.  Why?
	{                         // What is 'GSI'?  There are 166 rows in *.p file and 20 in param file
                GSI_vector[i] = 0.0;
	}
...
//Radiation parameters
	...
	treesParams.live_lai = treesParams.lai; // [sean] why two lai input values?  dead vs live LA (interception vs c-income/h2o)?
	treesParams.Al = treesParams.lai;
	treesParams.lai_at_sat_kl = treesParams.Al;
	//treesParams.Al = max(1.0, treesParams.lai);
...
//Initializing state variables for HydraulicModel
        for(j=0; j < n_tsteps; j++)
        {
                EcState.push_back(0.0);  //[sean] don't understand the syntax here. EcState is a vector of doubles
        }
        //set to 50 because model should take nowhere near this long to converge.
        for(j=0; j < 50; j++)
        {
                EcStateInner.push_back(0.0); //[sean] still don't undertand syntax ".push_back"
                LeafPsiInner.push_back(0.0); // but setting 50 index values to 0
                KpInner.push_back(0.0);
        }
        for(j=0; j < 50; j++)
        {
...
for (j = 0; j < treesParams.rmodules; j++)
{
	...
	if(j==1) { thetaSoil[j] = 0.2; }
	if(j==2) { thetaSoil[j] = 0.2; }
	if(j==3) { thetaSoil[j] = 0.2; }
	if(j==4) { thetaSoil[j] = 0.2; }
}
...
//define parameters for setup()
	...
	HydraulicModel::outputStruct ecrit_k_psi, k_p_e;
	// [sean]
	// ecritOut = minimum allowable e, given k and driving force
	// pc = leaf xylem pressure associated with Ecrit—the minimum permissible pressure w/o hydraulic failure
	// klpred = predicted whole plant k
	// ppredOut = is the outputted predicted leaf pressure based on midday E
	// epredOut = predicted transpiration rate based on the inputted midday xylem pressure
	// MD = max number of root+canopy layers. Set to 21
	// ULAT = max number of root soil layers
	// NREC = max number of output lines.  Set to 100,000
	// NMAX = max number of nodes (soil, I think).  Set to 121
	// NKINC = maximum number of increments of K for solving Weibulls.  Set to 15,000
	// I think 3d array 'psi' gives an array of pressures at each layer (canopy and soil)
	// I think 3d array 'rflux' is equivilent to Sperry's 'rhiz flux': relative water use from the different soil layers
	double ecritOut[MD], pc[MD], klpred[MD], ppredOut[MD], epredOut[MD], nodeFail[MD];
	char *nodeTyp = new char[MD];
	double ***psi, **rflux;
	if (treesParams.useHydraulics == true)
        {
    		psi = get3DArray(NREC,MD,ULAT); //[sean] i guess the dimensions can have unequal length
    		rflux = get2DArray(NREC,NMAX);
	}
...
//Reset K's to saturation
				...
				treesParams.forceRefilling = true;  // [sean] commenting out
			...
			{
				treesParams.forceRefilling = false;   // [sean] commenting out
				treesParams.xylemScalar = 1.0;
			}
			else
			{
...
//Initialize soil temp in each layer (up to ULAT layers)
                        for (j = 0; j < treesParams.rmodules; j++)
                        {
				if (j == 0) //[sean] wondering why this statement is indented like this...
                                {
                                        tempSoil[j] = 0.5*(t_surface+t_soil);
                                }
                                else if (j == 1)
...
//
				...
				{
					treesParams.forceRefilling = true;    // [sean] changing to false (default = true)
					treesParams.updatedHydraulics = true;
					//if (yday < 200)
					//if (yday < 252)
					if (yday < 182)
					{
						treesParams.allowLeafRefilling = true; //[sean] changing to false (default = true)
					}
					else
					{
						treesParams.allowLeafRefilling = true; //[sean] changing to true so refilling happens at all ages
					}
...
//for perennials call this once every 10 days; for annuals call every other day
						...
						(treesParams.usePhenology == false) )
					{
						...
						{
							psimin_in[mm][0] = psimin[mm][0];  //[sean] setting initial values for each module
...
//call simulations functions for current time step //[sean] this is where the main fxns are called...
                        simOut = simulation_functions(
