...
//trees_main.cpp
//
//main(): First function called when TREES is run
//
...
int main()
{
	// [sean]  enum is a user defined data type where we specify a set of values for a
	// variable and the variable can only take one out of a small set of possible values
	enum SizeFlag {SMALL, MEDIUM, LARGE};
	int i, j;
	String pfile, param_file_modules, dfile, cfile, sfile, hfile, lfile, efile, vfile; //filenames
	// [sean] ifstream allows for the reading of files
	ifstream fin;
	// [sean] ofstream allows for the writing of files
	ofstream chnout, effout, fluxout, hydrout, lfareaout;
	Data_Store indata; // [sean] user-defined class to store data
	State_Store state; // [sean] another user-defined class to store data
	...
	SizeFlag samplsize;
	Parameter* currp;  // [sean] pointer to user-defined class object to obtain parameter strings and vauels from *.p file
	Parameter* candp;
	double* curpval;
	double* candpval;
	double* minpval;
...
//[sean] changing this so input file, parameter file, and output file are hardcoded
//For hydraulic model
	struct trees_params treesParams; // [sean] declaring a struct object to hold data from *.p file
        int smodules;
        int rmodules;
...
//[sean] commenting out these two lines below
//  cout << "Input data file: ";
//  cin >> dfile;
//[sean] assigning text to string 'dfile'
	strcpy(dfile, "clim.txt");
...
*/
	...
	//[sean] commenting out these two lines...
//	cout << "Parameter file: ";
//	cin >> pfile;       // [sean] genotype-specific parameter data from *.p file saved in 'pfile'
	//[sean] changing to...
//	strcpy(pfile, "B97_new.p");
	strcpy(pfile, "B97_new.p");




	np = lines(pfile);  // [sean] assuming that 'lines' is a user-defined fxn giving the numbe of rows in pfile
	currp = new Parameter[np]; // [sean] assigning memory to int np..
	checkp = read_params(pfile, np, currp);  // passing to fxn... reads *.p file
	cout << pfile << " OK\n";
	rp = 0;  // [sean] this loop is just returning the data type from the *.p file?
	for(i=0;i<np;i++)
	{
		if(currp[i].get_dtype() == RANDOM) // [sean] random?
		{
			rp++;
		}
	}
...
//Plant Shoot and Root Module Parameter Read In
				...
				//[sean] commenting out the two lines below...
      	//  cout << "Shoot and Root Modules File Name: ";
      	//  cin >> param_file_modules;    //[sean] among other things, gives the number of shoot and root modules
				strcpy(param_file_modules, "param_mod");
        ...
        if(pmodules.is_open() == true)
        {
                ...
                treesParams.smodules = smodules;  // [sean] p vs s modules?  Plant vs soil?
...
//end user input
		...
		{
			// [sean] can't figure out what this weight is doing... but changed from 1 to 0 and simdensity is called each time
			weight = 1.0; //start with Ecframe in the likelihood function [sean] Ecframe = number of loops to aggregate
			// [sean] simdensity returns the probability distribution of MC runs...  allowing for variation in param values
...
//switch likelihood function each time covariance is updated
	...
	else if (rp == 0)//simulation with constant parameters [sean] ... assuming this is how we'll run most simulations
	{

		//[sean] commenting out these next two lines...
    //            cout << "Output file: ";
    //            cin >> sfile;
		//[sean] replace with... name of output file
		strcpy(sfile, "sean_output");
		...
		fluxout << "ti\tsimET\tWPlant_K\t";  // [sean] fluxout is a reference to ofstream (writes to file)
...
*/
		...
		// [sean] this appears to be the key code line (below) that calls to 'simulator', which calls to hydraulicModel
		// [sean] most of the code below and ouside the main fxn appears to be a summary of the sensitiivity analysis
		// [sean] if variation is introduced to the parameter input values
...
// [sean] this is the end of the main fxn... most of the code (without intorducing variation in params) just prints
// [sean] headers for each iteration... I think the simulator function runs the submodels to fill in data under the headers
