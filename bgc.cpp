...
void BiogeochemicalCycles::allocatePools()
{
	...
	Lf_idx = new int;  //[sean] is this an int or an int array?
	Lf_idx[0] = 0;     //[sean] set first index position to zero (avoids segfault in linux)
	// assert(Lf_idx != NULL);  //[sean] turning off this assert
...
void BiogeochemicalCycles::initializePools(trees_params treesParams)
{
	...
	for (int j = 0; j < nRoots; j++)
	{
		...
		for (int k = 0; k < nFineRootOrders; k++)
		{
			...
			rhizosphereNl[j][k] = rhizosphereCl[j][k] / (20.0+20.0*(1.0-1.0/SLAscalar)); //[sean] commenting out
			//cout << rhizosphereNl[j][k] << endl; //[sean]
			...
			// cout << rhizosphereNitrateNitrogen[j][k] << endl; //[sean]
		...
		nitrogenLeaching[j] = 0.0; //[sean] does zero mean zero leaching?
	...
	if (treesParams.useLeafModule == 1)
	{
		...
		//leafBiomassNitrogen[0] = max(0.1,(treesParams.leafLifeSpan-1.0)/treesParams.leafLifeSpan)*leafN;
		//leafBiomassNitrogen[0] =0.1;
		//leafBiomassNitrogen[0] = leafBiomassCarbon[0]/10.0;
		...
		leafStoredNitrogen[0] = leafNSC[0];
...
double BiogeochemicalCycles::C4photosynthesis(trees_params treesParams,
	...
	double& Cm)
	{
		...
		// cout << "Jmax = " << Jmax << "  Jmax25 = " << Jmax25 << endl; //[sean]
		// cout << gc << endl; //[sean]
		...
		int woo = 1; //[sean] add count to prevent endless loop
		while (stop == false && woo < 100) //sean added "&& woo < 100"
		{
		...
		// cout << "photo_count = " << woo << endl; //[sean]
		return An;
	}
	...
	void BiogeochemicalCycles::coupledA4_gc(trees_params treesParams,
		...
		double BiogeochemicalCycles::C3photosynthesis(trees_params treesParams,
			...
			void BiogeochemicalCycles::coupledA3_gc(trees_params treesParams,
				...
				{
					bool stop;
					double gc_min, gc_max, A_ref, dAdg;
					...
					//compute reference photosynthesis at the initial, maximum, stomatal conductance
					A_ref = C3photosynthesis(treesParams, Ca, Pa, Iin, phiJ, Tl, gc_max, Nmax, Ci, Cc, NG, NS);
					if (A_ref <= 0.0)
					{
						An = A_ref;
						gc = gc_min;
										...
										void BiogeochemicalCycles::computeMaximumRootBiomassCarbon(trees_params treesParams)
										{
											...
											for (int j = 0; j < nRoots; ++j)
											{
												...
												for (int k = 0; k < nRootOrders; ++k)
												{
													...
													//[sean] adding this to stop roots from growing into the bottom soil layer
				//									if(j == nRoots-1) {	allocationScalar /= 6.0; } //6 for no rorts, turn off for deep roots
											...
											void BiogeochemicalCycles::updateLateralRootWeibulls(double bsat[][MD],
												...
												double BiogeochemicalCycles::getRootCarbon(int j,
													...
													double BiogeochemicalCycles::getFineRootCarbon(int j,
														...
														{
															// cout << "leaf stored N = " << leafStoredNitrogen[0] << endl; //[sean]
	...
	void BiogeochemicalCycles::computeLeafAllocation(trees_params& treesParams,
		...
		double SLA)
		{
			...
			{
				unstressedLeafLifeSpan = treesParams.leafLifeSpan*365.25*48 * 10.0;
			}
			else
			{
				unstressedLeafLifeSpan = treesParams.leafLifeSpan*365.25*48.0*10.0;
			}
			//start with a base fraction of carbon allocation to leaf
			leafCfraction = 0.40*min(3.0,22.0/treesParams.SLA)/max(1.0, treesParams.leafLifeSpan);
			...
			// N_avail_rate_plant = 2; //[sean]
			// cout << N_avail_rate_plant << endl; //[sean]
...
//					if (yday > 210 || (yday > 203 && treesParams.live_lai > treesParams.lai_at_full_canopy_height))


					if (yday > 195) //[sean] turn off above line and replace with this one (14 days eralier GDU from 2600 to 2500)


					{
						//reproduction = rgrowth*N_avail_rate_plant*kratio;
						reproduction = rgrowth;
						// cout << "gbc_reproduction =" << reproduction << endl; //[sean] adding this
					}
					else
					{
						reproduction = 0.0;
					...
					//[sean] adding this reduce allocation as end of season approaches
					//[sean] for short-season maize start=250, end=295. Normal season maise start=265, end=313
					double senesce_start=265, season_end=313; //[sean] when fruit senescence starts and end of growth season (allocaiton to fruit=0)
					if(yday > senesce_start) {
						//[sean] winding back allocation to fruit as yday approaches the end of season
						reproduction = reproduction * cos((1 - (season_end - yday) / (season_end - senesce_start))*(3.14159*0.5));
						if(reproduction < 0) { reproduction = 0; }
																																	...
																																	void BiogeochemicalCycles::updateLeafCarbonNitrogenPools(trees_params& treesParams,
																																		...
																																		double& N_pos_demand)
																																		{
																																			...
																																			if (delta_lai > 0.0)
																																			{
																																				...
																																				CN = max(CN, 30.0);
																																				if (delta_nsc > 0.95*nsc)
																																				{
																																					delta_nsc = 0.95*nsc;
																																				}
																																				GRcost = delta_nsc * 0.14;
																																				total_N_demand = (delta_nsc-GRcost) / CN;
																																				if (total_N_demand > 0.95*leafStoredNitrogen[0])
																																				{
																																					NfromStorage = 0.95*leafStoredNitrogen[0];
																																		...
																																		void BiogeochemicalCycles::updateStemCarbonNitrogenPools(trees_params treesParams,
																																			...
																																			double SLA)
																																			{
																																				...
																																				// cout << "N_neg_demand = " << N_neg_demand << endl; //[sean]
																																				// cout << "N_pos_demand = " << N_pos_demand << endl; //[sean]
																																				// N_neg_demand = 0; //[sean]
																																				// N_pos_demand = 0; //[sean]
	...
	void BiogeochemicalCycles::updateRootCarbonNitrogenPools(trees_params treesParams,
		...
		double stemAllocation)
		{
			...
			for (int j = 0; j < nRoots; j++)
			{
				...
				for (int k = 0; k < nFineRootOrders; k++)
				{
					...
					//[sean] adding this to stop root from growing into the bottom-most soil layer
			//		if(j == nRoots-1) { rootAllocation = 0.001; }  // for no roots use 0.001.  For deep and dense use 0.1
					// cout << "j = " << j <<  "  rootAllocation = " << rootAllocation << endl;
					...
					//[sean] adding
					// cout << leafCfraction << '\t' << stemAllocation << '\t' << rgrowth << '\t' << tgrowth << '\t' << root_relative_growth << '\t' << rootAllocation << '\t' << CN << endl;

					//[sean] this if statement is causing the program to terminate
		...
		for (int j = 0; j < nRoots; j++)
		{
			...
			absorbFract = 1.0; //[sean] default=1..  If change to 0.02 then growth continues to end of simulation without segfault
			//[sean] this is associated with an increase in LAI and NPP and lower RLA
	...
	plantNstatus[0] = leafToRootCratio * leafStoredNitrogen[0]/(min(getLeafNSC(), treesParams.leafNSCscalar*leafBiomassCarbon[0])+0.000001);
	plantNstatus[0] += (1.0-leafToRootCratio)*getRootN()/(min(getRootNSC(), treesParams.leafNSCscalar*getFineRootCarbon())+0.000001);
	plantNstatus[0] *= 20.0*min(3.0, 22.0/treesParams.SLA);

	//cout << leafStoredNitrogen[0] << '\t' << getLeafNSC() << '\t' << leafBiomassCarbon[0] << '\t' << getRootN() << '\t' << getRootNSC() << '\t' << getFineRootCarbon() << endl;
	...
	double* bypassFlow)
	{
		...
		for (int j = 0; j < nRoots; j++)
		{
			...
			for (int k = 0; k < nRootOrders; k++)
			{
				...
				LE_neg[j][k] = rhizVolumeProportion * bypassFlow[j] * concentration; //[sean] add "*0" at end to eliminate leaching
				negLE[j] += LE_neg[j][k];
				//ammonium leaching
				a = 0.1; //assumes that most ammonium is absorbed in soil matrix
				concentration = 0.1 * a * rhizosphereAmmoniumNitrogen[j][k]/(rhizWaterVolume+0.0000001);
				LE_pos[j][k] = rhizVolumeProportion * bypassFlow[j] * concentration; //[sean] add "*0" at end to eliminate leaching
				posLE[j] += LE_pos[j][k];
			...
			// cout << nitrogenLeaching[j] << endl; //[sean]
																																																																					...
																																																																					double& N_pos_demand, double* Karray, double* Narray, double* rarray, int idx, double thetaRoot)
																																																																					{
																																																																						...
																																																																						((SingleLeafArea[k] < SingleLeafAreaPotential[k] ) ) )
																																																																						{
																																																																							...
																																																																							if (SingleLeafThermTm[k] > ((1.0-treesParams.proportion_CD)* duration_Leaf))
																																																																							{
																																																																								...
																																																																								/*
																																																																								// update plant-level leaf nitrogen pools, adapted from computeleafNdemand() function used in phenology module

																																																																								NfromStorage = deltaN/cm2_to_ha(treesParams.pot_size) ; // kgN/ha
																																																																						...
																																																																						double BiogeochemicalCycles::calcPlantSLA(int Lf_idx,
																																																																							trees_params treesParams)
																																																																							{
																																																																								...
																																																																								return(sum_area/(sum_C*10000.0)); // SLA in m2/kg C
																																																																							}
